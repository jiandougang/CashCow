...
class ControllerCover(context: Context) : BaseCover(context), OnTimerUpdateListener, OnTouchGestureListener, View.OnClickListener {
    ...
    private val mOnGroupValueUpdateListener = object : IReceiverGroup.OnGroupValueUpdateListener {
        ...
        override fun onValueUpdate(key: String, value: Any) {
            if (key == DataInter.Key.KEY_COMPLETE_SHOW) {
                ...
                if (value as Boolean){
                    setScreenSwitchEnable(false)
                }else{
                    setScreenSwitchEnable(true)
                }
    ...
    override fun onClick(view: View) {
        when (view.id) {
            R.id.cover_player_controller_image_view_back_icon ->
                notifyReceiverEvent(DataInter.Event.EVENT_CODE_REQUEST_BACK, Bundle())
            ...
            R.id.cover_player_controller_image_view_switch_screen ->{
                ...
                notifyReceiverEvent(DataInter.Event.EVENT_CODE_REQUEST_TOGGLE_SCREEN,  Bundle())
    ...
    private fun setSwitchScreenIcon(isFullScreen: Boolean) {
        mSwitchScreen!!.setImageResource(if (isFullScreen) R.mipmap.icon_video_exit_full_screen else R.mipmap.icon_video_full_screen)
    }
    ...
    private fun setTopContainerState(state: Boolean) {
        if (mControllerTopEnable) {
            ...
            mTopAnimator = ObjectAnimator.ofFloat(mTopContainer,
                    "alpha", if (state) 0f else 1f, if (state) 1f else 0f).setDuration(500)
    ...
    override fun onTimerUpdate(curr: Int, duration: Int) {
    ...
    override fun onPlayerEvent(eventCode: Int, bundle: Bundle?) {
        when (eventCode) {
            ...
            OnPlayerEventListener.PLAYER_EVENT_ON_BUFFERING_UPDATE -> if (bundle != null) {
    ...
    override fun onCreateCoverView(context: Context): View? {
        return View.inflate(context, R.layout.widget_controller_cover, null)
    }
