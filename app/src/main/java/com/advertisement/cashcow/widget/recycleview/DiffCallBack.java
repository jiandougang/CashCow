package com.advertisement.cashcow.widget.recycleview;
...
public class DiffCallBack<T> extends DiffUtil.Callback {
    ...
    public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
        ...
        //默认两个data内容是相同的,如果有内容不同，就返回false
        if (beanOld == beanNew){
            return true;
        }else {
            return false;
    ...
    public Object getChangePayload(int oldItemPosition, int newItemPosition) {
        // 定向刷新中的部分更新
        // 效率最高
        //返回不为null的话，onChange采用Partial bind，就不会出现白光一闪onChange动画。 反之就有。
        T oldBean = mOldDatas.get(oldItemPosition);
        T newBean = mNewDatas.get(newItemPosition);

        //这里就不用比较核心字段了,一定相等
        Bundle payload = new Bundle();
        if (oldBean == newBean) {
            payload.putString("KEY_DESC", newBean.toString());
        }
